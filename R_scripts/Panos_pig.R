...
#Final
heatmap_data = read.csv("/Users/pbanerjee/Documents/CBU/CBU_Projects/Neuroscience/Panos/Pig/Deseq2_files/Heatmap_data_all_genes_vst.csv", sep = ",")
heatmap_data <- heatmap_data[c(1,5,9,12)]
heatmap_data_no_duplicates <- heatmap_data[!duplicated(heatmap_data$hgnc_symbol),]
row.names(heatmap_data_no_duplicates) <- heatmap_data_no_duplicates$hgnc_symbol
# non-unique values when setting 'row.names': ‘’, ‘SNORA50A’ 
heatmap_data <- subset(heatmap_data_no_duplicates, select = -c(hgnc_symbol))
heatmap_data <- as.matrix(heatmap_data)
# rename
colnames(heatmap_data)[1] <- "Normoxic"
colnames(heatmap_data)[2] <- "Hypoxic-Ischemic"
colnames(heatmap_data)[3]<- "EGFRi"
# reorder
heatmap_reorder <- heatmap_data[, c("Normoxic","Hypoxic-Ischemic","EGFRi")]
heatmap_pig_all <- pheatmap(heatmap_reorder, main = "Gene Expression", 
         show_rownames=FALSE, cluster_rows=T, cluster_cols=F, show_colnames = T,
         angle_col = 0,
         breaks = seq(-0.5, +0.5, length = 101))
heatmap_pig_all
save_plot("/Users/pbanerjee/Documents/CBU/CBU_Projects/Neuroscience/Panos/Pig/Heatmaps/heatmap_pig_all_final.png",heatmap_pig_all)
...
heatmap_pig_all_with_no_0_gene_expression <- pheatmap(non_zero_expression_genes_apply_mehod, main = "Gene Expression", 
                                                      show_rownames=FALSE, cluster_rows=T, cluster_cols=F, show_colnames = T,
                                                      angle_col = 0,
                                                      breaks = seq(-0.5,+0.5, length = 101))
heatmap_pig_all_with_no_0_gene_expression
save_plot("/Users/pbanerjee/Documents/CBU/CBU_Projects/Neuroscience/Panos/Pig/Heatmaps/heatmap_pig_all_with_no_0_gene_expression.png",heatmap_pig_all_with_no_0_gene_expression)
write.table(non_zero_expression_genes_apply_mehod, "/Users/pbanerjee/Documents/CBU/CBU_Projects/Neuroscience/Panos/Pig/Heatmaps/heatmap_pig_all_with_no_0_gene_expression_genes.txt", sep = "\t")

# # Final heatmap
# heatmap_data = read.table("/Users/pbanerjee/Documents/CBU/CBU_Projects/Neuroscience/Panos/Mouse/Heatmaps/Heatmap_mouse_data_all_genes_vst.txt", sep = "\t", header = TRUE)
...
#          show_rownames=FALSE, cluster_rows=T, cluster_cols=F, show_colnames = T,
#          angle_col = 0,
#          breaks = seq(-0.5, +0.5, length = 100)) 

# # Final heatmap - previous 800 pic
...
# heatmap_data <- as.matrix(heatmap_data)
# # rename
# colnames(heatmap_data)[1]<- "Gene Name"
# colnames(heatmap_data)[2] <- "Normoxic"
# 
# colnames(heatmap_data)[1] <- "Normoxic"
...
pheatmap(syn_data_ensg, main = "Synaptic Transmission",
         show_rownames=T, cluster_rows=T, cluster_cols=F, show_colnames = T,
         angle_col = 0,
...
pheatmap(nerv_data_ensg, main = "Nervous System",
         show_rownames=T, cluster_rows=T, cluster_cols=F, show_colnames = T,
         angle_col = 0, cex = 0.7,
         breaks = seq(-1, +1, length = 101))
